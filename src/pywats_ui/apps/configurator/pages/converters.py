"""
Converters Page V2 (pyWATS UI Migration)

Unified converter management with reliability improvements:
...
- H1: Error handling for all user operations
- H3: Event loop guards for async operations
- H4: Proper cleanup on page destruction
- C1: QueueManager integration where applicable
- M1: Retry logic for file operations
"""
...
def get_system_converters() -> List[ConverterInfo]:
    ...
    try:
        from pywats_client.converters.standard import (
            SeicaXMLConverter,
        ...
        system_converters = [
            ...
            ("Teradyne Spectrum ICT", "TeradyneSpectrumICTConverter", "teradyne_spectrum_ict_converter",
        ...
        # H1: Handle import errors gracefully
        print(f"Warning: Could not import system converters: {e}")
...
def create_default_converter_configs(data_path: Path) -> List[ConverterConfig]:
    ...
    converters_data = [
        ...
        ("Teradyne Spectrum ICT", "pywats_client.converters.standard.teradyne_spectrum_ict_converter.TeradyneSpectrumICTConverter",
    ...
    for name, module_path, folder_name, patterns, description in converters_data:
        ...
        try:
            # M1: Retry folder creation
            watch_folder.mkdir(parents=True, exist_ok=True)
            done_folder.mkdir(exist_ok=True)
            error_folder.mkdir(exist_ok=True)
            pending_folder.mkdir(exist_ok=True)
        except Exception as e:
            # H1: Log error but continue with other folders
            print(f"Warning: Could not create folders for {name}: {e}")
            continue
    ...
    try:
        ...
        for py_file in user_folder.glob("*.py"):
            ...
            try:
                ...
                # H1: Skip malformed files
                print(f"Warning: Could not parse {py_file.name}: {e}")
                pass
    except Exception as e:
        # H1: Handle glob errors
        print(f"Warning: Could not list user converters: {e}")
...
class ConverterSettingsDialogV2(QDialog):
    ...
    def _on_accept(self) -> None:
        """Validate and accept (H1: with error handling, M1: with retries)"""
    ...
    def closeEvent(self, event):
        """Cleanup on close"""
        # No resources to clean up in this dialog
...
class ConverterEditorDialogV2(QDialog):
    ...
    - Syntax highlighting
    - Version tracking
    - H1: Error handling for file operations
    - H4: Cleanup on close
    """
    ...
    def _setup_ui(self) -> None:
        ...
        # Script editor (fallback to simple text editor if widget not available)
        try:
            from pywats_client.gui.widgets import ScriptEditorWidget
            self._script_editor = ScriptEditorWidget()
            self._script_editor.content_changed.connect(self._on_content_changed)
        except ImportError:
            # H1: Fallback to simple QPlainTextEdit
            self._script_editor = QPlainTextEdit()
            self._script_editor.setReadOnly(self.is_system)
            self._script_editor.textChanged.connect(self._on_content_changed)
    ...
    def _load_converter(self) -> None:
        """Load converter source (H1: with error handling)"""
        try:
            if hasattr(self._script_editor, 'load_file'):
                if self._script_editor.load_file(str(self.converter_info.file_path)):
                    if self.is_system:
                        # Make editor read-only for system converters
                        if hasattr(self._script_editor, '_code_editor'):
                            self._script_editor._code_editor.setReadOnly(True)
            else:
                # Fallback for QPlainTextEdit
                content = self.converter_info.file_path.read_text(encoding='utf-8')
    ...
    def _on_save_new_version(self) -> None:
        ...
        try:
            ...
            if ok and new_version:
                ...
                if hasattr(self._script_editor, 'get_source'):
                ...
                if hasattr(self._script_editor, 'load_source'):
                    ...
                    # Fallback
                    self._script_editor.setPlainText(updated)
...
'''
    ...
    def _on_close(self) -> None:
        ...
        try:
            is_modified = False
            if hasattr(self._script_editor, 'is_modified'):
                is_modified = self._script_editor.is_modified()
            elif isinstance(self._script_editor, QPlainTextEdit):
...
class ConvertersPageV2(BasePage):
    ...
    - H1: Error handling for all operations
    - H3: Event loop guards (not applicable - no async operations)
    - H4: Proper cleanup on page destruction
    - C1: QueueManager integration (not applicable for this page)
    - M1: Retry logic for file operations
    """
    ...
    def __init__(
        self, 
        config: ClientConfig, 
        main_window: Optional['ConfiguratorMainWindow'] = None,
        parent: Optional[QWidget] = None
    ) -> None:
    ...
    def load_config(self) -> None:
        """Load configuration (H1: with error handling)"""
        try:
    ...
    def _on_browse_folder(self) -> None:
        """Browse for user converters folder (H1: with error handling)"""
        try:
    ...
    def _on_add_config(self, conv: ConverterInfo) -> None:
        """Add configuration for a converter (H1: with error handling)"""
        try:
    ...
    def _on_new_converter(self) -> None:
        ...
        try:
            ...
            try:
                from pywats_client.gui.widgets import NewConverterDialog
                ...
                if dialog.exec() == QDialog.DialogCode.Accepted:
                    ...
                    if hasattr(dialog, 'created_path') and dialog.created_path:
    ...
    def cleanup(self) -> None:
        """Cleanup resources"""
        # No specific resources to clean up (no async tasks, no monitors)
        pass

from collections.abc import Mapping
from typing import Any, TypeVar, Optional, BinaryIO, TextIO, TYPE_CHECKING, Generator

from attrs import define as _attrs_define
from attrs import field as _attrs_field

from ..types import UNSET, Unset

from ..types import UNSET, Unset
from dateutil.parser import isoparse
from typing import cast
from typing import Union
from uuid import UUID
import datetime

if TYPE_CHECKING:
  from ..models.virinco_wats_web_dashboard_models_mes_software_package_folder import VirincoWATSWebDashboardModelsMesSoftwarePackageFolder
  from ..models.virinco_wats_web_dashboard_models_mes_virtual_folder import VirincoWATSWebDashboardModelsMesVirtualFolder
  from ..models.virinco_wats_web_dashboard_models_mes_software_package_site_relation import VirincoWATSWebDashboardModelsMesSoftwarePackageSiteRelation





T = TypeVar("T", bound="VirincoWATSWebDashboardModelsMesSoftwarePackage")



@_attrs_define
class VirincoWATSWebDashboardModelsMesSoftwarePackage:
    """ 
        Attributes:
            package_id (Union[Unset, UUID]):  Example: 00000000-0000-0000-0000-000000000000.
            name (Union[Unset, str]):
            package_folder (Union[Unset, bool]):
            version (Union[Unset, int]):
            description (Union[Unset, str]):
            root_directory (Union[Unset, str]):
            tags (Union[Unset, str]):
            modified (Union[Unset, datetime.datetime]):
            modified_by (Union[Unset, str]):
            status (Union[Unset, int]):
            priority (Union[Unset, int]):
            virtual_folder_id (Union[Unset, UUID]):  Example: 00000000-0000-0000-0000-000000000000.
            virtual_folder (Union[Unset, VirincoWATSWebDashboardModelsMesVirtualFolder]):
            package_folders (Union[Unset, list['VirincoWATSWebDashboardModelsMesSoftwarePackageFolder']]):
            package_site_relations (Union[Unset, list['VirincoWATSWebDashboardModelsMesSoftwarePackageSiteRelation']]):
            is_restricted (Union[Unset, bool]):
            validation (Union[Unset, str]):
            validation_description (Union[Unset, str]):
            created (Union[Unset, datetime.datetime]):
            created_by (Union[Unset, str]):
            released (Union[Unset, datetime.datetime]):
            released_by (Union[Unset, str]):
            revoked (Union[Unset, datetime.datetime]):
            revoked_by (Union[Unset, str]):
     """

    package_id: Union[Unset, UUID] = UNSET
    name: Union[Unset, str] = UNSET
    package_folder: Union[Unset, bool] = UNSET
    version: Union[Unset, int] = UNSET
    description: Union[Unset, str] = UNSET
    root_directory: Union[Unset, str] = UNSET
    tags: Union[Unset, str] = UNSET
    modified: Union[Unset, datetime.datetime] = UNSET
    modified_by: Union[Unset, str] = UNSET
    status: Union[Unset, int] = UNSET
    priority: Union[Unset, int] = UNSET
    virtual_folder_id: Union[Unset, UUID] = UNSET
    virtual_folder: Union[Unset, 'VirincoWATSWebDashboardModelsMesVirtualFolder'] = UNSET
    package_folders: Union[Unset, list['VirincoWATSWebDashboardModelsMesSoftwarePackageFolder']] = UNSET
    package_site_relations: Union[Unset, list['VirincoWATSWebDashboardModelsMesSoftwarePackageSiteRelation']] = UNSET
    is_restricted: Union[Unset, bool] = UNSET
    validation: Union[Unset, str] = UNSET
    validation_description: Union[Unset, str] = UNSET
    created: Union[Unset, datetime.datetime] = UNSET
    created_by: Union[Unset, str] = UNSET
    released: Union[Unset, datetime.datetime] = UNSET
    released_by: Union[Unset, str] = UNSET
    revoked: Union[Unset, datetime.datetime] = UNSET
    revoked_by: Union[Unset, str] = UNSET
    additional_properties: dict[str, Any] = _attrs_field(init=False, factory=dict)





    def to_dict(self) -> dict[str, Any]:
        from ..models.virinco_wats_web_dashboard_models_mes_software_package_folder import VirincoWATSWebDashboardModelsMesSoftwarePackageFolder
        from ..models.virinco_wats_web_dashboard_models_mes_virtual_folder import VirincoWATSWebDashboardModelsMesVirtualFolder
        from ..models.virinco_wats_web_dashboard_models_mes_software_package_site_relation import VirincoWATSWebDashboardModelsMesSoftwarePackageSiteRelation
        package_id: Union[Unset, str] = UNSET
        if not isinstance(self.package_id, Unset):
            package_id = str(self.package_id)

        name = self.name

        package_folder = self.package_folder

        version = self.version

        description = self.description

        root_directory = self.root_directory

        tags = self.tags

        modified: Union[Unset, str] = UNSET
        if not isinstance(self.modified, Unset):
            modified = self.modified.isoformat()

        modified_by = self.modified_by

        status = self.status

        priority = self.priority

        virtual_folder_id: Union[Unset, str] = UNSET
        if not isinstance(self.virtual_folder_id, Unset):
            virtual_folder_id = str(self.virtual_folder_id)

        virtual_folder: Union[Unset, dict[str, Any]] = UNSET
        if not isinstance(self.virtual_folder, Unset):
            virtual_folder = self.virtual_folder.to_dict()

        package_folders: Union[Unset, list[dict[str, Any]]] = UNSET
        if not isinstance(self.package_folders, Unset):
            package_folders = []
            for package_folders_item_data in self.package_folders:
                package_folders_item = package_folders_item_data.to_dict()
                package_folders.append(package_folders_item)



        package_site_relations: Union[Unset, list[dict[str, Any]]] = UNSET
        if not isinstance(self.package_site_relations, Unset):
            package_site_relations = []
            for package_site_relations_item_data in self.package_site_relations:
                package_site_relations_item = package_site_relations_item_data.to_dict()
                package_site_relations.append(package_site_relations_item)



        is_restricted = self.is_restricted

        validation = self.validation

        validation_description = self.validation_description

        created: Union[Unset, str] = UNSET
        if not isinstance(self.created, Unset):
            created = self.created.isoformat()

        created_by = self.created_by

        released: Union[Unset, str] = UNSET
        if not isinstance(self.released, Unset):
            released = self.released.isoformat()

        released_by = self.released_by

        revoked: Union[Unset, str] = UNSET
        if not isinstance(self.revoked, Unset):
            revoked = self.revoked.isoformat()

        revoked_by = self.revoked_by


        field_dict: dict[str, Any] = {}
        field_dict.update(self.additional_properties)
        field_dict.update({
        })
        if package_id is not UNSET:
            field_dict["PackageId"] = package_id
        if name is not UNSET:
            field_dict["Name"] = name
        if package_folder is not UNSET:
            field_dict["PackageFolder"] = package_folder
        if version is not UNSET:
            field_dict["Version"] = version
        if description is not UNSET:
            field_dict["Description"] = description
        if root_directory is not UNSET:
            field_dict["RootDirectory"] = root_directory
        if tags is not UNSET:
            field_dict["Tags"] = tags
        if modified is not UNSET:
            field_dict["Modified"] = modified
        if modified_by is not UNSET:
            field_dict["ModifiedBy"] = modified_by
        if status is not UNSET:
            field_dict["Status"] = status
        if priority is not UNSET:
            field_dict["Priority"] = priority
        if virtual_folder_id is not UNSET:
            field_dict["VirtualFolderId"] = virtual_folder_id
        if virtual_folder is not UNSET:
            field_dict["VirtualFolder"] = virtual_folder
        if package_folders is not UNSET:
            field_dict["PackageFolders"] = package_folders
        if package_site_relations is not UNSET:
            field_dict["PackageSiteRelations"] = package_site_relations
        if is_restricted is not UNSET:
            field_dict["IsRestricted"] = is_restricted
        if validation is not UNSET:
            field_dict["Validation"] = validation
        if validation_description is not UNSET:
            field_dict["ValidationDescription"] = validation_description
        if created is not UNSET:
            field_dict["Created"] = created
        if created_by is not UNSET:
            field_dict["CreatedBy"] = created_by
        if released is not UNSET:
            field_dict["Released"] = released
        if released_by is not UNSET:
            field_dict["ReleasedBy"] = released_by
        if revoked is not UNSET:
            field_dict["Revoked"] = revoked
        if revoked_by is not UNSET:
            field_dict["RevokedBy"] = revoked_by

        return field_dict



    @classmethod
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        from ..models.virinco_wats_web_dashboard_models_mes_software_package_folder import VirincoWATSWebDashboardModelsMesSoftwarePackageFolder
        from ..models.virinco_wats_web_dashboard_models_mes_virtual_folder import VirincoWATSWebDashboardModelsMesVirtualFolder
        from ..models.virinco_wats_web_dashboard_models_mes_software_package_site_relation import VirincoWATSWebDashboardModelsMesSoftwarePackageSiteRelation
        d = dict(src_dict)
        _package_id = d.pop("PackageId", UNSET)
        package_id: Union[Unset, UUID]
        if isinstance(_package_id,  Unset):
            package_id = UNSET
        else:
            package_id = UUID(_package_id)




        name = d.pop("Name", UNSET)

        package_folder = d.pop("PackageFolder", UNSET)

        version = d.pop("Version", UNSET)

        description = d.pop("Description", UNSET)

        root_directory = d.pop("RootDirectory", UNSET)

        tags = d.pop("Tags", UNSET)

        _modified = d.pop("Modified", UNSET)
        modified: Union[Unset, datetime.datetime]
        if isinstance(_modified,  Unset):
            modified = UNSET
        else:
            modified = isoparse(_modified)




        modified_by = d.pop("ModifiedBy", UNSET)

        status = d.pop("Status", UNSET)

        priority = d.pop("Priority", UNSET)

        _virtual_folder_id = d.pop("VirtualFolderId", UNSET)
        virtual_folder_id: Union[Unset, UUID]
        if isinstance(_virtual_folder_id,  Unset):
            virtual_folder_id = UNSET
        else:
            virtual_folder_id = UUID(_virtual_folder_id)




        _virtual_folder = d.pop("VirtualFolder", UNSET)
        virtual_folder: Union[Unset, VirincoWATSWebDashboardModelsMesVirtualFolder]
        if isinstance(_virtual_folder,  Unset):
            virtual_folder = UNSET
        else:
            virtual_folder = VirincoWATSWebDashboardModelsMesVirtualFolder.from_dict(_virtual_folder)




        package_folders = []
        _package_folders = d.pop("PackageFolders", UNSET)
        for package_folders_item_data in (_package_folders or []):
            package_folders_item = VirincoWATSWebDashboardModelsMesSoftwarePackageFolder.from_dict(package_folders_item_data)



            package_folders.append(package_folders_item)


        package_site_relations = []
        _package_site_relations = d.pop("PackageSiteRelations", UNSET)
        for package_site_relations_item_data in (_package_site_relations or []):
            package_site_relations_item = VirincoWATSWebDashboardModelsMesSoftwarePackageSiteRelation.from_dict(package_site_relations_item_data)



            package_site_relations.append(package_site_relations_item)


        is_restricted = d.pop("IsRestricted", UNSET)

        validation = d.pop("Validation", UNSET)

        validation_description = d.pop("ValidationDescription", UNSET)

        _created = d.pop("Created", UNSET)
        created: Union[Unset, datetime.datetime]
        if isinstance(_created,  Unset):
            created = UNSET
        else:
            created = isoparse(_created)




        created_by = d.pop("CreatedBy", UNSET)

        _released = d.pop("Released", UNSET)
        released: Union[Unset, datetime.datetime]
        if isinstance(_released,  Unset):
            released = UNSET
        else:
            released = isoparse(_released)




        released_by = d.pop("ReleasedBy", UNSET)

        _revoked = d.pop("Revoked", UNSET)
        revoked: Union[Unset, datetime.datetime]
        if isinstance(_revoked,  Unset):
            revoked = UNSET
        else:
            revoked = isoparse(_revoked)




        revoked_by = d.pop("RevokedBy", UNSET)

        virinco_wats_web_dashboard_models_mes_software_package = cls(
            package_id=package_id,
            name=name,
            package_folder=package_folder,
            version=version,
            description=description,
            root_directory=root_directory,
            tags=tags,
            modified=modified,
            modified_by=modified_by,
            status=status,
            priority=priority,
            virtual_folder_id=virtual_folder_id,
            virtual_folder=virtual_folder,
            package_folders=package_folders,
            package_site_relations=package_site_relations,
            is_restricted=is_restricted,
            validation=validation,
            validation_description=validation_description,
            created=created,
            created_by=created_by,
            released=released,
            released_by=released_by,
            revoked=revoked,
            revoked_by=revoked_by,
        )


        virinco_wats_web_dashboard_models_mes_software_package.additional_properties = d
        return virinco_wats_web_dashboard_models_mes_software_package

    @property
    def additional_keys(self) -> list[str]:
        return list(self.additional_properties.keys())

    def __getitem__(self, key: str) -> Any:
        return self.additional_properties[key]

    def __setitem__(self, key: str, value: Any) -> None:
        self.additional_properties[key] = value

    def __delitem__(self, key: str) -> None:
        del self.additional_properties[key]

    def __contains__(self, key: str) -> bool:
        return key in self.additional_properties

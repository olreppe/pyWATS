from collections.abc import Mapping
from typing import Any, TypeVar, Optional, BinaryIO, TextIO, TYPE_CHECKING, Generator

from attrs import define as _attrs_define
from attrs import field as _attrs_field

from ..types import UNSET, Unset

from ..types import UNSET, Unset
from typing import Union






T = TypeVar("T", bound="VirincoWATSWebDashboardModelsMesCulture")



@_attrs_define
class VirincoWATSWebDashboardModelsMesCulture:
    """ 
        Attributes:
            culture_id (Union[Unset, int]):
            culture_code (Union[Unset, str]):
            culture_name (Union[Unset, str]):
     """

    culture_id: Union[Unset, int] = UNSET
    culture_code: Union[Unset, str] = UNSET
    culture_name: Union[Unset, str] = UNSET
    additional_properties: dict[str, Any] = _attrs_field(init=False, factory=dict)





    def to_dict(self) -> dict[str, Any]:
        culture_id = self.culture_id

        culture_code = self.culture_code

        culture_name = self.culture_name


        field_dict: dict[str, Any] = {}
        field_dict.update(self.additional_properties)
        field_dict.update({
        })
        if culture_id is not UNSET:
            field_dict["cultureId"] = culture_id
        if culture_code is not UNSET:
            field_dict["cultureCode"] = culture_code
        if culture_name is not UNSET:
            field_dict["cultureName"] = culture_name

        return field_dict



    @classmethod
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        d = dict(src_dict)
        culture_id = d.pop("cultureId", UNSET)

        culture_code = d.pop("cultureCode", UNSET)

        culture_name = d.pop("cultureName", UNSET)

        virinco_wats_web_dashboard_models_mes_culture = cls(
            culture_id=culture_id,
            culture_code=culture_code,
            culture_name=culture_name,
        )


        virinco_wats_web_dashboard_models_mes_culture.additional_properties = d
        return virinco_wats_web_dashboard_models_mes_culture

    @property
    def additional_keys(self) -> list[str]:
        return list(self.additional_properties.keys())

    def __getitem__(self, key: str) -> Any:
        return self.additional_properties[key]

    def __setitem__(self, key: str, value: Any) -> None:
        self.additional_properties[key] = value

    def __delitem__(self, key: str) -> None:
        del self.additional_properties[key]

    def __contains__(self, key: str) -> bool:
        return key in self.additional_properties

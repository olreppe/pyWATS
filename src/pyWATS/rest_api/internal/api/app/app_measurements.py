...
from ...models.app_measurements_response_200 import AppMeasurementsResponse200
...
def _parse_response(*, client: Union[AuthenticatedClient, Client], response: httpx.Response) -> Optional[AppMeasurementsResponse200]:
    if response.status_code == 200:
        response_200 = AppMeasurementsResponse200.from_dict(response.json())
...
def _build_response(*, client: Union[AuthenticatedClient, Client], response: httpx.Response) -> Response[AppMeasurementsResponse200]:
    return Response(
...
def sync_detailed(
    *,
...
) -> Response[AppMeasurementsResponse200]:
    """ PREVIEW - Get numeric measurements by measurement path. A maximum of 10000 measurments are returned.
    ...
    Separet multiple paths with semi colon (;)
    ex.
...
) -> Optional[AppMeasurementsResponse200]:
    """ PREVIEW - Get numeric measurements by measurement path. A maximum of 10000 measurments are returned.
    Requesting the endpoint with a empty filter will return measurements from the last seven days most
    failed steps

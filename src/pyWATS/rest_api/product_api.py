...
class ProductApi:
    ...
    def create_or_update_product(self, product: Union[Product, Dict[str, Any]]) -> Optional[Product]:
        """
    ...
    def create_or_update_product_revision(
    ...
    def bulk_create_or_update_products(
        ...
        """
        data = [
            p.model_dump(by_alias=True, exclude_none=True) if isinstance(p, Product) else p
            for p in products
        ]
    ...
    def bulk_create_or_update_revisions(
        ...
        """
        data = [
            r.model_dump(by_alias=True, exclude_none=True) if isinstance(r, ProductRevision) else r
            for r in revisions
        ]
    ...
    def get_product_groups_by_product(self, part_number: str, revision: str) -> List[ProductGroup]:
        """
        ...
        Returns:
            List of ProductGroup objects
        """
        response = self._http.get(f"/api/Product/Groups/{part_number}/{revision}")
        if response.is_success and response.data:
            return [ProductGroup.model_validate(item) for item in response.data]
        return []
    ...
    def get_vendors(self) -> List[Dict[str, Any]]:
        ...
        Returns:
            List of vendor dictionaries (no dedicated model)
        """

"""
TDM Analytics Models

Data models for TDM analytics operations, providing structures for statistical analysis,
trend data, and analytical results.

Extracted from original tdm/models.py for better organization.
"""
...
class TopFailedData(BaseModel):
    """Top failed data for analytics."""
    step_name: str = Field(..., alias="stepName")
    failure_count: int = Field(..., alias="failureCount")
    failure_rate: float = Field(..., alias="failureRate")
    part_number: Optional[str] = Field(None, alias="partNumber")
...
class YieldTrendData(BaseModel):
    """Yield trend analysis data model."""
    part_number: str = Field(..., alias="partNumber")
    trend_points: List[TrendDataPoint] = Field([], alias="trendPoints")
    average_yield: float = Field(0.0, alias="averageYield")
    best_yield: float = Field(0.0, alias="bestYield")
    worst_yield: float = Field(0.0, alias="worstYield")
    
    class Config:
        allow_population_by_field_name = True


class ProcessCapabilityData(BaseModel):
    """Process capability analysis data model."""
    measurement_name: str = Field(..., alias="measurementName")
    cp: Optional[float] = None  # Process capability index
    cpk: Optional[float] = None  # Process capability index (adjusted)
    ...
    process_mean: Optional[float] = Field(None, alias="processMean")
    process_std: Optional[float] = Field(None, alias="processStd")

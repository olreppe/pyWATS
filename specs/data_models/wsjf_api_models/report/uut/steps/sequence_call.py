...
from ..steps import *
from .numeric_step import NumericStep, MultiNumericStep, MultiNumericMeasurement, NumericMeasurement
#from .string_step import StringMeasurement, MultiStringMeasurement
...
class SequenceCallInfo(BaseModel):
    ...
    file_name: Optional[str] = Field(default=None, max_length=200, min_length=1, deserialization_alias="name", serialization_alias="name")
    version: Optional[str] = Field(default=None, max_length=30, min_length=1)

    # -------------------------------------------------------------------
    # Model validator steps(before) - Provides replacements for None in required fields when deserializing.
...
class SequenceCall(Step):
    ...
    """
    step_type: Literal["SequenceCall"] = Field(default="SequenceCall",deserialization_alias="stepType", serialization_alias="stepType")
    sequence: SequenceCallInfo = Field(default_factory=SequenceCallInfo, deserialization_alias="seqCall", serialization_alias="seqCall")
    ...
    def add_string_step(self, *,
        ...
        ss.add_measurement(value=value, unit=unit, status=status, comp_op=comp_op, limit=limit)
    ...
    def add_boolean_step(self, *,
        ...
        bs.add_measurement(status=status)
    ...
    model_config = {
        ...
        "json_encoders": {StepList: list}  # Tell Pydantic how to serialize StepList
    }

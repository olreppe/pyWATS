...
class MiscInfo(BaseModel):
    ...
    numeric_value: Optional[int] = Field(default=None,
                                         deprecated=True, 
                                         deserialization_alias = "numeric",
                                         serialization_alias = "numeric",
                                         description="Numeric value. Not available for analysis - use string_value")
    
    type_def: Optional[str] = Field(default=None, max_length=30, min_length=0, deserialization_alias="typedef", serialization_alias="typedef",
                                    description="??????????????????????????????????????????????")    
    numeric_normat: Optional[str] = Field(default=None, deprecated=True, deserialization_alias="numericFormat", serialization_alias="numericFormat",
                                          description="?????????????????????????????")
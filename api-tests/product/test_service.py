...
from pywats.domains.product.service import ProductService
...
class DummyProductRepository:
    ...
    def get_by_part_number(self, part_number: str) -> Product:
    ...
    def save_revision(self, revision: ProductRevision) -> ProductRevision:
...
def test_create_product_persists_via_repository(product_service: ProductService) -> None:
...
def test_create_revision_attaches_product(product_service: ProductService) -> None:
    repository = product_service._repository

    revision = product_service.create_revision(
        part_number="PN-999",
        revision="R1",
        name="Revision 1"
    )
...
def test_get_products_returns_views(product_service: ProductService) -> None:
    views = product_service.get_products()

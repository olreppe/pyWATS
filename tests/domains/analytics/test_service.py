...
class DummyAnalyticsRepository:
    """Mock repository for testing AnalyticsService."""
    ...
    async def get_serial_number_history(self, filter_data: WATSFilter) -> List[ReportHeader]:
...
async def test_get_dynamic_yield_passes_filters(analytics_service: AnalyticsService) -> None:
    ...
    result = await analytics_service.get_dynamic_yield(filter_data)

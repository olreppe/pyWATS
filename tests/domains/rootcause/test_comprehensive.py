...
"""
    ...
    # API test user - assign ticket so status can be changed
    test_user = "ola.lund.reppe@virinco.com"
    ...
    if ticket:
        ...
        # Assign ticket after creation (required before changing status)
        assigned = wats_client.rootcause.assign_ticket(ticket.ticket_id, test_user)
        if assigned:
            ticket = assigned
    ...
    return {
        ...
        "defect": defect,
        "subject": subject,
        "assignee": test_user,  # Store assignee for use in tests
    }
...
"""
        ...
        wats_client.rootcause.add_comment(
            ...
            comment=d1_note,
            assignee=rootcause_ticket["assignee"]
        )
...
"""
        ...
        if updated:
            print(f"  [OK] D8 Team Recognition documented")
...
class TestTicketArchival:
    ...
    def test_archive_solved_ticket(
        ...
        # Try to archive - may fail due to permissions or server rules
        try:
            ...
            # Archive may fail due to permissions or business rules
            print(f"  [NOTE] Archive failed: {e}")
            print(f"  [NOTE] This may be due to permissions or server configuration")
        ...
        if archived_ticket:
            ...
            if status_val == TicketStatus.ARCHIVED.value:
                ...
                print(f"  [NOTE] Ticket status is {status_val}, archive may require different permissions")
        
        print("======================\n")
        
        # Test passes as long as ticket still exists

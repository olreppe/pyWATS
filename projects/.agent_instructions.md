...
## 📅 **TIMESTAMPING REQUIREMENT** (NEW - February 14, 2026)

**ALL working documents MUST include timestamps to track currency:**

### File Naming Convention
- **Projects**: `MMDDHHMM-{project-name}.project/`
  - Example: `02140900-api-performance-optimization.project/`
  - Format: Month-Day-Hour-Minute of completion/archival
  
### Document Headers (REQUIRED)
Every working document must have a header with:
```markdown
...
### When to Timestamp
- ✅ **ALWAYS**: Analysis docs, implementation plans, status summaries
- ✅ **ALWAYS**: Architecture decisions, design docs, RFC-style proposals
- ✅ **ALWAYS**: Meeting notes, session summaries, progress reports
- ⚠️ **SOMETIMES**: Test files (if they represent a snapshot of test strategy)
- ⚠️ **SOMETIMES**: Temporary working notes (if they might be referenced later)
- ❌ **NEVER**: Source code (use git history instead)
- ❌ **NEVER**: Auto-generated docs (e.g., Sphinx API docs)

### Timestamp Update Rules
1. Update `Last Updated` when making significant changes
2. Mark as `Outdated` when information is superseded by newer docs
3. Move to `projects/completed/{quarter}/` when project finishes
4. Delete outdated docs during quarterly cleanup if no longer useful
...
4. On completion, agent moves to projects/completed/{quarter}/
...
└── {project-name}/
    ...
    ├── 01_ANALYSIS.md              # Problem analysis, requirements, constraints
    ├── 02_IMPLEMENTATION_PLAN.md   # Step-by-step implementation plan
    ├── 03_PROGRESS.md              # Real-time progress tracking
    ├── 04_TODO.md                  # Task checklist (✅ ✗ 🚧)
    ├── tests/                      # Project-specific tests
    ├── scripts/                    # Automation scripts
    └── notes/                      # Working notes, discoveries
```
...
**STEP 4: Implementation Phase** ⚠️ CRITICAL: Keep TODO.md Updated
```
FOR EACH TASK:
  1. Mark task as 🚧 In Progress in TODO.md (BEFORE starting work)
  2. Implement the change
  3. Update PROGRESS.md with what was done
  4. Mark task as ✅ Completed in TODO.md (IMMEDIATELY after completion)
  5. Run tests and verify
  6. Update README.md status if phase complete
  
⚠️ CRASH RECOVERY: TODO.md is your recovery point!
   - Update TODO.md DURING work, not at the end
   - If session crashes, TODO.md shows exactly where you were
   - Mark items complete AS YOU GO, not in batch at end
   - Each TODO update = checkpoint for recovery
```
...
## 🎨 Agent Prompt Patterns (USE THESE)

### Starting a New Task (ALWAYS update TODO.md first!)
```
"Based on 02_IMPLEMENTATION_PLAN.md, I will now implement Phase 1, Step 1.2: {task name}.

⚠️ Updating 04_TODO.md to mark this task as 🚧 In Progress BEFORE starting work.
This ensures crash recovery knows exactly where we are."
```

### After Completing a Task (IMMEDIATELY update TODO.md!)
```
"Completed: {task description}
Files modified: {list files}
Tests passing: {X/Y}

⚠️ Updating IMMEDIATELY (not batching):
- 04_TODO.md to mark task ✅ Complete (CRASH RECOVERY CHECKPOINT)
...
Reason: {explanation}
Moving to 04_TODO.md ⏸️ Blocked section.
...
## 🔀 Alternative: GitHub Issues Workflow (For Larger Projects)
...
### DO:
✅ Always create all 4 required markdown files  
✅ **Update TODO.md after EVERY task completion** (not in batches!)  
✅ **Update TODO.md BEFORE starting each task** (mark as 🚧 In Progress)  
✅ Update PROGRESS.md with timestamps  
✅ Cross-link documents at the top  
✅ Ask for clarification when requirements unclear  
✅ Run tests after each implementation step  
✅ Commit frequently with descriptive messages  
✅ Move completed projects to docs/internal_documentation/completed/  
✅ **Treat TODO.md as your recovery checkpoint** - update continuously

### DON'T:
❌ Skip the analysis phase  
❌ Start coding before creating TODO.md  
❌ **Batch-update TODO.md at end of session** (update AS YOU GO!)  
❌ Forget to update progress after completing tasks  
❌ Mix multiple unrelated changes in one commit  
❌ Leave projects in limbo (mark as complete or cancelled)  
❌ Delete project folders (move to completed or archive)  
❌ **Forget to mark tasks in-progress before starting** (recovery depends on it!)  
...
| Complete | ✅ | All success criteria met, tests passing |
| Blocked | ⏸️ | Waiting on external dependency |
| Cancelled | ❌ | Project abandoned/superseded |
...
- [ ] **Move to examples/**: Copy useful examples to appropriate `examples/` subdirectory
  ...
  - Report examples → `examples/report/`
  - General usage → `examples/` (root)
...
### 5️⃣ Changelog Updates ⚠️ CRITICAL
...
- [ ] **Update README.md status**: Change to "✅ Complete"

### 8️⃣ Commit and Archive
...
1. ✅ All tests passing
2. 📋 Move tests from `projects/{name}/tests/` → `tests/`
3. 📖 Move examples from `projects/{name}/examples/` → `examples/`
4. 📝 Update CHANGELOG.md under `[Unreleased]`
5. 🗑️ Delete project `tests/` and `examples/` folders (already moved)
6. 📄 Create COMPLETION_SUMMARY.md
7. 💾 Commit, push, move to `completed/{quarter}/`
...
A: Update README status to "🔶 On Hold", document reason, commit changes.

**Q: Project cancelled mid-way?**  
A: Update README status to "❌ Cancelled", document why, move to archive/.

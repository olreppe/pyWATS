...
- [Alarm and Notification Logs](#alarm-and-notification-logs-internal-api)
...
WATS supports 5 alarm types:

| Type | Enum | Description |
|------|------|-------------|
| 1 | `AlarmType.REPORT` | Unit test result alarms (e.g., "Unit not passed after 5 retests") |
| 2 | `AlarmType.YIELD_VOLUME` | Yield/volume threshold alarms (e.g., "FPY drops by 10%") |
| 3 | `AlarmType.SERIAL_NUMBER` | Serial number pool monitoring |
| 4 | `AlarmType.MEASUREMENT` | Measurement/SPC alarms (Cp/Cpk thresholds) |
| 5 | `AlarmType.ASSET` | Asset status and maintenance alarms |

### Basic Usage

```python
from pywats import pyWATS, AlarmType
from datetime import datetime, timedelta
...
for alarm in yield_alarms:
    print(f"{alarm.name}: FPY={alarm.fpy_percent:.1f}%, Trend={alarm.fpy_trend_percent:+.1f}%")
```

### AlarmLog Model Fields

The `AlarmLog` model contains different fields depending on the alarm type:

**Common fields (all types):**
- `log_id`: Unique identifier
- `name`: Alarm rule name
- `log_date`: When the alarm triggered
- `type`: Alarm type (1-5)
- `state`: Alarm state
- `part_number`, `product_name`, `process_code`, `process_name`

**Type 1 (REPORT) fields:**
- `report_guid`: Related test report GUID
...
**Type 2 (YIELD_VOLUME) fields:**
- `fpy`, `spy`, `tpy`, `lpy`: Yield values (0-1)
- `fpy_trend`, `spy_trend`, etc.: Trend changes
- `unit_count`, `uut_count`, `retest_count`: Volume metrics
- Helper properties: `fpy_percent`, `fpy_trend_percent`

**Type 3 (SERIAL_NUMBER) fields:**
- `free`: Available serial numbers
- `reserved`: Reserved serial numbers
- `serial_number_type`: Type name

**Type 4 (MEASUREMENT) fields:**
- `cp`, `cpk`: Process capability indices
- `min`, `max`, `avg`, `stdev`: Statistical values
- `measurement_path`: Path to the measurement

**Type 5 (ASSET) fields:**
- `asset_name`, `asset_serial_number`: Asset identification
- `type_id`: Asset type
- `asset_state`, `bubbled_status`: Status information
- `tags`: Asset tags

### Building an Alarm Monitor

pyWATS provides a comprehensive example for building a polling-based alarm monitor. See `examples/analytics/alarm_monitor.py` for:

- Configurable polling loop
- State tracking to detect new alarms
- Multiple notification handlers (Console, Email, Slack, Teams)
...
See `examples/analytics/alarm_monitor.py` for a production-ready implementation with proper state tracking.

...
namespace Virinco.WATS.Interface.MES.Software
{
    ...
    public class Software : MesBase
    {
        ...
        private long validateFiles(IEnumerable<PackageFolder> packageFolders, DirectoryInfo parentFolder)
        {
            ...
            foreach (PackageFolder packageFolder in packageFolders)
            {
                ...
                foreach (PackageFolderFile pfile in packageFolder.PackageFolderFiles)
                {
                    ...
                    else
                    {
                        ...
                        else if (fileInfo.Exists && fileInfo.LastWriteTimeUtc != pfile.RepositoryFolderFile.FileModifiedDate)
                        {
                            //Don't count files that have changed but are OverwriteNever
                            if (pfile.RepositoryFolderFile.FileSize.HasValue && !FileAttributeContainsOneOf((FileAttribute)pfile.Attributes, FileAttribute.OverwriteNever, FileAttribute.OverwriteOnNewPackageVersion))
                                total += pfile.RepositoryFolderFile.FileSize.Value;
                        }
                    }
                }
            ...
            bool FileAttributeContainsOneOf(FileAttribute source, params FileAttribute[] flags)
            {
                for (int i = 0; i < flags.Length; i++)
                {
                    if (source.HasFlag(flags[i]))
                        return true;
                }
        ...
        internal void storePackageInfo(Package package, bool markAsInstalled = false)// Guid PackageGuid, string PackageName, string FolderName)
        {
            try
            {
                ...
                else
                {
                    ...
                    var xTags = xPackage.Element("Tags");
                    if (xTags == null)
                    {
                        xTags = new XElement("Tags");
                        xPackage.Add(xTags);
                    }
                    else
                    {
                    ...
                    xTags.Add(XElement.Parse(package.Tags));
                }
        ...
        private void ExecuteFile(FileInfo file, bool waitForExecution)
        {
            try
            {
                ...
                if (file.Exists)
                {
                    using (System.Diagnostics.Process p = Utilities.UnsafeOpenFileInSystemDefaultProgram(file.FullName))
                    {

            ...
            foreach (PackageFolder packageFolder in packageFolders)
            {
                try
                {
                    ...
                    foreach (PackageFolderFile pfile in packageFolder.PackageFolderFiles)
                    {
                        ...
                        {
                            string fullname = folder.FullName + "\\" + pfile.RepositoryFolderFile.Name;
                            if (fullname.Length > 256)
                                throw new PathTooLongException($"Package file path is too long: {fullname}");
                            else
                                throw;
                        }
                    }
                ...
                {
                    var directoryPath = parentFolder.FullName + "\\" + packageFolder.Name;
                    if (directoryPath.Length > 256)
                        throw new PathTooLongException($"Package folder path is too long: {directoryPath}");
                    else
                        throw;
                }
            }

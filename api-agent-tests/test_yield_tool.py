...
class TestRealLLMQueryExecution:
    ...
    def test_top_10_runners_real_execution(self, wats_client):
        """
        REAL TEST: Execute 'Give me top 10 runners' query against actual WATS server.
        
        This test:
        1. Simulates LLM extracting parameters from "Give me top 10 runners"
        2. Executes the yield tool with real API
        ...
        tool_input = {
            "top_count": 10,
            "perspective": "by_product",  # Group by product/part number
            "sort_by": "unit_count",  # Runners = high volume
            "period_count": 30
        }
        
        print(f"\nLLM Query: 'Give me top 10 runners'")
        print(f"Extracted Parameters: {tool_input}")
        
        # EXECUTE - This makes a REAL API call
    ...
    def test_yield_by_station_last_7_days_real(self, wats_client):
        ...
        print(f"\nLLM Query: 'Show yield by station for last 7 days'")
        print(f"Extracted Parameters: {tool_input}")
    ...
    def test_worst_performers_last_30_days_real(self, wats_client):
        ...
        tool_input = {
            "top_count": 10,
            "perspective": "by_product",
            "sort_by": "fpy",
            "ascending": True,  # Worst first
            "period_count": 30
        }
        
        print(f"\nLLM Query: 'Show me the 10 worst performing products in the last 30 days'")
        print(f"Extracted Parameters: {tool_input}")
